...
class Kmer_sorter_stxxl{
    private:

    Kmer_sorter_stxxl(const Kmer_sorter_stxxl&) = delete; // Forbid copying because we have a pointer member
    void operator=(const Kmer_sorter_stxxl&) = delete; // Forbid copying because we have a pointer member

    // define comparator class:
    struct Compare{
        // comparison operator() returning true if (a < b)
        inline bool operator () (pair<Kmer, payload_t> a, pair<Kmer, payload_t> b) const{
            return a.first < b.first;
        }

        pair<Kmer, payload_t> min_value() const { 
            return {Kmer(""), payload_t()}; // Todo: use pre-allocated kmer
        }
        pair<Kmer, payload_t> max_value() const {
            return {Kmer("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT"), payload_t()}; // Todo: use pre-allocated kmer
        }
    };

    Compare comparator;
    typedef stxxl::sorter<pair<Kmer, payload_t>, Compare> sr_counter_type;
    sr_counter_type* sr_counter;
    bool is_sorted = false;

    public:

    Kmer_sorter_stxxl() {
        sr_counter = new sr_counter_type(comparator, 1024*1024*10);
    }
    ...
    // WARNING!!!! This deletes the sorted and creates a new one. Call only right after constructor
    // Has to be done this way because stxxl does not allow changing the memory budget after initialization
    void set_mem_budget(LL bytes){
        assert(!is_sorted);
        delete sr_counter;
        sr_counter = new sr_counter_type(comparator, max(bytes, (LL)1024*1024*10));
        // Need to give at least 10MB or else stxxl complains about insufficient memory
    }
    ...
    bool stream_done() const{
        return sr_counter->empty();
    }

    pair<Kmer, payload_t> stream_next(){
        assert(is_sorted);
        pair<Kmer, payload_t> ret = **sr_counter; // Derefence pointer and then dereference iterator
        ++(*sr_counter);
        return ret;
    }

    pair<Kmer, payload_t> peek_next(){
        return **sr_counter; // Derefence pointer and then dereference iterator
    }
};
...
private:
    ...
    LL process_prefixes(Kmer x, Edgeset E, Kmer_sorter_stxxl<Edgeset>& new_sorter){
        LL k = x.get_k();
        LL count = 0;
    ...
    LL add_dummies(Kmer_sorter_stxxl<Edgeset>& old_sorter, Kmer_sorter_stxxl<Edgeset>& new_sorter){
        string ACGT = "ACGT";
        Kmer prev_kmer;
        Edgeset cur_edgeset;
        LL loopcount = 0;
...
public:
    ...
    boss_t build(edgemer_stream& input){
        ...
        Kmer_sorter_stxxl<Edgeset> sorter1;
        ...
        // Dummies
        Kmer_sorter_stxxl<Edgeset> sorter2;
        ...
        // Build BOSS vectors
        Kmer_stream_merger<Edgeset, Kmer_sorter_stxxl<Edgeset>> merger(sorter1, sorter2);
